...
#输出灵敏度
'''
def show_specificity(predictLabel,Label):
    TN = 0
    FP = 0
    #label_1 = np.zeros(Label.shape[0])
    predlabel = []
    label_1 = Label.argmax(axis = 1)
    predlabel = predictLabel.argmax(axis = 1)
    for i in list(range(Label.shape[0])):
        if label_1[i] == 0 and predlabel[i]==0:
            TN += 1
        if label_1[i] == 0 and predlabel[i]==1:
            FP += 1
    return (round(TN/(TN+FP),5))
'''
#输出特异性
'''
def show_sensitivity(predictLabel,Label):
    TP = 0
    FN = 0
    #label_1 = np.zeros(Label.shape[0])
    predlabel = []
    ...
    predlabel = predictLabel.argmax(axis = 1)
    for i in list(range(Label.shape[0])):
        if label_1[i] == 1 and predlabel[i]==1:
            TP += 1
        if label_1[i] == 1 and predlabel[i]==0:
            FN += 1
    return (round(TP/(TP+FN),5))
'''
激活函数
'''
def tansig(x):
...
#    u = 0
    ...
    train_spec = np.zeros([1, L + 1])
    train_sen = np.zeros([1, L + 1])
    train_time = np.zeros([1,L+1])
    test_acc = np.zeros([1,L+1])
    test_spec = np.zeros([1, L + 1])
    test_sen = np.zeros([1, L + 1])
...
#        print('Feature nodes in window: max:',np.max(outputOfEachWindow),'min:',np.min(outputOfEachWindow))
        ...
        ##SE block
        outputOfEachWindow = np.array(outputOfEachWindow)
        x = outputOfEachWindow.reshape(-1, 4, 9,845)  # 针对某一数据集改变宽度学习网络矩阵形状，以便于其能够进入到通道注意力机制SE
        out_se = SE.se_block(x)  # 获取SE的输出结果
        out_se = np.reshape(out_se, (845, N1))  # 转化矩阵形状以回到宽度学习网络
        outputOfEachWindow = out_se
...
#    print('Enhance nodes: max:',np.max(tempOfOutputOfEnhanceLayer),'min:',np.min(tempOfOutputOfEnhanceLayer))
    ...
    trainSpec = show_specificity(OutputOfTrain, train_y)
    trainSen = show_sensitivity(OutputOfTrain, train_y)
    print('Training accurate is' ,trainAcc*100,'%')
    print('Training specificity is', trainSpec * 100, '%')
    print('Training sensitivity is', trainSen * 100, '%')
    print('Training time is ',trainTime,'s')
    train_acc_all[0][0] = trainAcc
    train_spec[0][0] = trainSpec
    train_sen[0][0] = trainSen
...
#  最终测试输出
    ...
    testSen = show_sensitivity(OutputOfTest, test_y)
    testSpec = show_specificity(OutputOfTest, test_y)
    testAcc = show_accuracy(OutputOfTest,test_y)
    print('Testing accurate is' ,testAcc * 100,'%')
    print('Testing specificity is', testSpec * 100, '%')
    print('Testing Sensitivity is', testSen * 100, '%')
    print('Testing time is ',testTime,'s')
    test_acc[0][0] = testAcc
    test_sen[0][0] = testSen
    test_spec[0][0] = testSpec
